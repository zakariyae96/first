...
def create_user(name: str, email: str, phone: int, password: str) -> Optional[User]:
    if find_user(email):
        return None
    user = User()
    user.name = name
...
def get_profile(usr: str) -> Optional[User]:
    ...
    user_profile = graph.run(f"MATCH (x:user) WHERE x.email='{usr}' RETURN x.name as name, x.phone as phone, x.email as email").data()
    return user_profile
